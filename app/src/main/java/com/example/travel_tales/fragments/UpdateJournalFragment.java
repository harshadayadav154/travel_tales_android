...
import com.example.travel_tales.databinding.FragmentUpdateJournalBinding;
 ...
 * Use the {@link UpdateJournalFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class UpdateJournalFragment extends Fragment implements View.OnClickListener, Geocoder.GeocodeListener {

    private JournalMini journalMini;
    private FragmentUpdateJournalBinding binding; //View binding object
    ...
    public UpdateJournalFragment(JournalMini journalMini) {
        this.journalMini = journalMini;
    }
     ...
     */
    public static UpdateJournalFragment newInstance(String param1, String param2) {
        UpdateJournalFragment fragment = new UpdateJournalFragment();
    ...
    private void initializeComponents() {
        ...
        try {
            journalEntry = dbHelper.getJournalEntryById(journalMini.getId());
        } catch (Exception ignore) {
        }
        geocoder = new Geocoder(UpdateJournalFragment.this.requireContext());
    ...
    /**
     * Initializes the view binding with data from the JournalEntry object.
     * If the JournalEntry object is not null, it populates the UI fields
     * with the corresponding data.
     */
    private void initBinding() {
        if (journalEntry != null) {
            // Set the title of the journal entry to the EditText
            binding.editTextTitle.setText(journalEntry.getTitle());

            // Set the formatted date of the journal entry to the EditText
            binding.editTextDate.setText(DateUtility.formatDateToString(journalEntry.getDate()));

            // If the journal entry has a location, set its name to the EditText
            if (journalEntry.getLocation() != null) {
                this.location = journalEntry.getLocation();
                binding.editTextAddressSearch.setText(journalEntry.getLocation().getName());
            }

            // Set the description of the journal entry to the EditText
            binding.editTextDescription.setText(journalEntry.getDescription());

            // Set the image paths of the journal entry
            imagePaths = journalEntry.getImagePaths();
    ...
    private void processImages(ClipData clipData) {
        executorService.execute(() -> {
            ...
            requireActivity().runOnUiThread(this::switchToProgressBarView);
            ...
            for (int i = 0; i < clipData.getItemCount(); i++) {
                ...
                try {
                    ...
                    Bitmap bitmap = ImageUtility.getBitmapFromUri(requireContext(), uri);
                    String imagePath = ImageUtility.saveImageToInternalStorage(requireContext(), bitmap, uri.getLastPathSegment());
            ...
            getActivity().runOnUiThread(() -> {
                if (!imagePaths.isEmpty()) {
                    ...
                    if (!validImagePaths.isEmpty()) {
                        ...
                        // No valid image paths, hiding preview and progress bar
                        binding.imgPreview.setVisibility(View.GONE);
                        binding.progressBar.setVisibility(View.GONE);
                    }
                }
    ...
    private void processFormSubmission() throws ParseException {
        ...
        NotificationUtility.showRecordSuccessNotification(getContext(), NotificationUtility.RecordOperation.UPDATE);
    }
